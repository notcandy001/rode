# NixOS module for Rode
{ config, lib, pkgs, ... }:

let
  cfg = config.programs.rode;
in {
  options.programs.rode = {
    enable = lib.mkEnableOption "Rode shell";

    package = lib.mkOption {
      type = lib.types.package;
      description = "The Rode package to use";
    };

    fonts.enable = lib.mkOption {
      type = lib.types.bool;
      default = true;
      description = "Whether to install Rode fonts (including Phosphor Icons)";
    };
  };
