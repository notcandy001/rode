#!/usr/bin/env bash
# Rode CLI - It was needed, so here it is. lol
...
ensure_config_files() {
	local config_dir="${XDG_CONFIG_HOME:-$HOME/.config}/rode/config"
	local preset_dir="${SCRIPT_DIR}/assets/presets/Rode Default"
...
show_help() {
	cat <<EOF
Rode CLI - Desktop Environment Control
...
Commands:
    ...
    version, -v, --version            Show Rode version
    goodbye                           Uninstall Rode :(
    install <target>                    Install compositor config (hyprland)
    remove <target>                    Remove compositor config (hyprland)

Examples:
    rode brightness 75              Set all monitors to 75%
    rode brightness 50 HDMI-A-1     Set HDMI-A-1 to 50%
    rode brightness +10             Increase brightness by 10%
    rode brightness -5 HDMI-A-1     Decrease HDMI-A-1 brightness by 5%
    rode brightness 10 -s           Save current, then set all to 10%
    rode brightness -s HDMI-A-1     Save current brightness of HDMI-A-1
    rode brightness -r              Restore saved brightness

EOF
}

RODE_HYPR_SOURCE="source = ~/.local/share/rode/hyprland.conf"
RODE_HYPR_BLOCK=$(
	cat <<'EOF'
# Rode
...
append_rode_hyprland_block() {
	local conf="$1"

	if [ -f "$conf" ] && grep -qF "$RODE_HYPR_SOURCE" "$conf"; then
		echo "Rode Hyprland block already present in $conf"
		return 0
	fi

	if [ -f "$conf" ] && [ -s "$conf" ]; then
		printf "\n%s\n" "$RODE_HYPR_BLOCK" >>"$conf"
	else
		printf "%s\n" "$RODE_HYPR_BLOCK" >"$conf"
	fi

	echo "Added Rode Hyprland block to $conf"
}

remove_rode_hyprland_block() {
	local conf="$1"
	...
	awk -v source="$RODE_HYPR_SOURCE" '
		function is_remove(line) {
			return line == source \
				|| line == "# Rode" \
				|| line == "# OVERRIDES" \
				|| line == "# Down here you can write or source anything that you want to override from Rode'\''s settings."
		}
		{
			lines[NR] = $0
		}
	...
	echo "Removed Rode Hyprland block from $conf"
}
...
restart_rode() {
	# Kill rodctl processes first (they survive parent death when forked/detached)
	pkill -f "rodctl.*daemon" 2>/dev/null || true
	pkill -f "rodctl subscribe" 2>/dev/null || true

	PID=$(find_rode_pid_cached)
	if [ -n "$PID" ]; then
		echo "Stopping Rode (PID $PID)..."
		kill "$PID"
...
update)
	echo "Updating Rode..."
	curl -fsSL github.com/notcandy/rode/releases/latest | sh
	restart_rode
	;;
refresh)
	echo "Refreshing Rode profile..."
	exec nix profile upgrade Rode --refresh --impure
	;;
run)
	CMD="${2:-}"
	PIPE="/tmp/rode_ipc.pipe"
	...
	PID=$(find_rode_pid_cached)
	if [ -z "$PID" ]; then
		echo "Error: Rode is not running"
		exit 1
	fi

	qs ipc --pid "$PID" call rode run "$CMD" 2>/dev/null || {
...
lock)
	...
	qs ipc --pid "$PID" call rode run lockscreen 2>/dev/null || {
...
brightness)
	...
	BRIGHTNESS_SAVE_FILE="/tmp/rode_brightness_saved.txt"
	...
	if [[ "$ARG2" =~ ^[0-9]+$ ]]; then
		...
		echo "Run 'rode help' for usage information"
		exit 1
	fi
	...
	;;
version | -v | --version)
	echo "Rode $(cat "${SCRIPT_DIR}/version")"
	;;
install)
	TARGET="${2:-}"
	if [ "$TARGET" = "hyprland" ]; then
		...
		append_rode_hyprland_block "$HYPR_CONF"
	else
...
remove)
	...
	if [ "$TARGET" = "hyprland" ]; then
		...
		remove_rode_hyprland_block "$HYPR_CONF"
	else
		echo "Error: Unknown target '$TARGET'. Supported: hyprland"
		exit 1
	fi
...
goodbye)
	...
	if [ -f /etc/NIXOS ]; then
		if nix profile list 2>/dev/null | grep -q "Rode"; then
			echo "Removing from nix profile..."
			nix profile remove Rode
		elif command -v rode >/dev/null 2>&1; then
			echo "Rode was declared in this system. Please remove it from your configuration in order to uninstall."
		else
			echo "Rode is not installed."
		fi
		exit 0
	fi
	...
	rm -rf "$HOME/.local/src/rode"
	rm -rf "$HOME/.local/share/rode"
	rm -rf "$HOME/.local/state/rode"

	if [ "$REMOVE_CONFIG" = true ]; then
		rm -rf "$HOME/.config/rode"
		echo "Configuration files removed."
	fi

	echo "Rode uninstalled. :("
	;;
help | --help | -h)
	show_help
	;;
