...
Singleton {
    ...
    property string configDir: (Quickshell.env("XDG_CONFIG_HOME") || (Quickshell.env("HOME") + "/.config")) + "/rode/config"
    property string keybindsPath: (Quickshell.env("XDG_CONFIG_HOME") || (Quickshell.env("HOME") + "/.config")) + "/rode/binds.json"
    property string presetDir: Qt.resolvedUrl("../assets/presets/Rode Default").toString().replace("file://", "")
    ...
    FileView {
        ...
        adapter: JsonAdapter {
            ...
            property string customText: "Rode"
        ...
        adapter: JsonAdapter {
            ...
            property JsonObject idle: JsonObject {
                property JsonObject general: JsonObject {
                    property string lock_cmd: "rode lock"
                    property string before_sleep_cmd: "loginctl lock-session"
                    property string after_sleep_cmd: "rode screen on"
                }
                property list<var> listeners: [
                    {
                        "timeout": 150,
                        "onTimeout": "rode brightness 10 -s",
                        "onResume": "rode brightness -r"
                    },
                    {
                        "timeout": 300,
                    ...
                    {
                        "timeout": 330,
                        "onTimeout": "rode screen off",
                        "onResume": "rode screen on"
                    },
                    {
                        "timeout": 1800,
                        "onTimeout": "rode suspend"
                    }
                ]
            }
    ...
    function repairKeybinds() {
        ...
        try {
            ...
            if (current.rode.dashboard && typeof current.rode.dashboard === "object" && !current.rode.dashboard.modifiers) {
                console.log("Migrating nested rode binds to flat structure...");
                const nested = current.rode.dashboard;
                
                // Map old names to new names and update arguments
                if (nested.widgets) {
                    current.rode.launcher = nested.widgets;
                    current.rode.launcher.argument = "rode run launcher";
                    current.rode.launcher.action = createAction(current.rode.launcher);
                }
                if (nested.dashboard) {
                    current.rode.dashboard = nested.dashboard;
                    current.rode.dashboard.argument = "rode run dashboard";
                    current.rode.dashboard.action = createAction(current.rode.dashboard);
                }
                if (nested.assistant) {
                    current.rode.assistant = nested.assistant;
                    current.rode.assistant.argument = "rode run assistant";
                    current.rode.assistant.action = createAction(current.rode.assistant);
                }
                if (nested.clipboard) {
                    current.rode.clipboard = nested.clipboard;
                    current.rode.clipboard.argument = "rode run clipboard";
                    current.rode.clipboard.action = createAction(current.rode.clipboard);
                }
                if (nested.emoji) {
                    current.rode.emoji = nested.emoji;
                    current.rode.emoji.argument = "rode run emoji";
                    current.rode.emoji.action = createAction(current.rode.emoji);
                }
                if (nested.notes) {
                    current.rode.notes = nested.notes;
                    current.rode.notes.argument = "rode run notes";
                    current.rode.notes.action = createAction(current.rode.notes);
                }
                if (nested.tmux) {
                    current.rode.tmux = nested.tmux;
                    current.rode.tmux.argument = "rode run tmux";
                    current.rode.tmux.action = createAction(current.rode.tmux);
                }
                if (nested.wallpapers) {
                    current.rode.wallpapers = nested.wallpapers;
                    current.rode.wallpapers.argument = "rode run wallpapers";
                    current.rode.wallpapers.action = createAction(current.rode.wallpapers);
                }

                // Remove the old nested object
                delete current.rode.dashboard;
                needsUpdate = true;
            }

            if (!current.rode.system) {
                current.rode.system = {};
                needsUpdate = true;
            }
            ...
            if (!adapter || !adapter.rode) return;
            ...
            // Check rode core binds
            const rodeKeys = ["launcher", "dashboard", "assistant", "clipboard", "emoji", "notes", "tmux", "wallpapers"];
            for (const key of rodeKeys) {
                if (!current.rode[key] && adapter.rode[key]) {
                    console.log("Adding missing rode bind:", key);
                    current.rode[key] = createCleanBind(adapter.rode[key]);
                    needsUpdate = true;
                } else if (current.rode[key] && !current.rode[key].action) {
                    current.rode[key].action = createAction(current.rode[key]);
                    delete current.rode[key].dispatcher;
                    delete current.rode[key].argument;
                    delete current.rode[key].flags;
                    needsUpdate = true;
                }
            }
            ...
            for (const key of systemKeys) {
                if (!current.rode.system[key] && adapter.rode.system && adapter.rode.system[key]) {
                    console.log("Adding missing system bind:", key);
                    current.rode.system[key] = createCleanBind(adapter.rode.system[key]);
                    needsUpdate = true;
                } else if (current.rode.system[key] && !current.rode.system[key].action) {
                    current.rode.system[key].action = createAction(current.rode.system[key]);
                    delete current.rode.system[key].dispatcher;
                    delete current.rode.system[key].argument;
                    delete current.rode.system[key].flags;
                    needsUpdate = true;
                }
            }
        ...
        adapter: JsonAdapter {
            ...
            property JsonObject system: JsonObject {
                property JsonObject config: JsonObject {
                    ...
                    property var action: ({ "id": "rode.config", "args": {} })
                }
                ...
                property JsonObject overview: JsonObject {
                    ...
                    property var action: ({ "id": "rode.overview", "args": {} })
                }
                property JsonObject powermenu: JsonObject {
                    property list<string> modifiers: ["SUPER"]
                    property string key: "ESCAPE"
                    property var action: ({ "id": "rode.powermenu", "args": {} })
                }
                property JsonObject tools: JsonObject {
                    property list<string> modifiers: ["SUPER"]
                    property string key: "S"
                    property var action: ({ "id": "rode.tools", "args": {} })
                }
                property JsonObject screenshot: JsonObject {
                    property list<string> modifiers: ["SUPER", "SHIFT"]
                    property string key: "S"
                    property var action: ({ "id": "rode.screenshot", "args": {} })
                }
                property JsonObject screenrecord: JsonObject {
                    property list<string> modifiers: ["SUPER", "SHIFT"]
                    property string key: "R"
                    property var action: ({ "id": "rode.screenrecord", "args": {} })
                }
                property JsonObject lens: JsonObject {
                    property list<string> modifiers: ["SUPER", "SHIFT"]
                    property string key: "A"
                    property var action: ({ "id": "rode.lens", "args": {} })
                }
                property JsonObject reload: JsonObject {
                    property list<string> modifiers: ["SUPER", "ALT"]
                    property string key: "B"
                    property var action: ({ "id": "rode.reload", "args": {} })
                }
                property JsonObject quit: JsonObject {
                    property list<string> modifiers: ["SUPER", "CTRL", "ALT"]
                    property string key: "B"
                    property var action: ({ "id": "rode.quit", "args": {} })
                }
            }
            }
            // Default getters
            readonly property var defaultRodeBinds: {
                "rode": {
                    "launcher": { "modifiers": ["SUPER"], "key": "Super_L", "action": { "id": "rode.launcher", "args": {} } },
                    "dashboard": { "modifiers": ["SUPER"], "key": "D", "action": { "id": "rode.dashboard", "args": {} } },
                    "assistant": { "modifiers": ["SUPER"], "key": "A", "action": { "id": "rode.assistant", "args": {} } },
                    "clipboard": { "modifiers": ["SUPER"], "key": "V", "action": { "id": "rode.clipboard", "args": {} } },
                    "emoji": { "modifiers": ["SUPER"], "key": "PERIOD", "action": { "id": "rode.emoji", "args": {} } },
                    "notes": { "modifiers": ["SUPER"], "key": "N", "action": { "id": "rode.notes", "args": {} } },
                    "tmux": { "modifiers": ["SUPER"], "key": "T", "action": { "id": "rode.tmux", "args": {} } },
                    "wallpapers": { "modifiers": ["SUPER"], "key": "COMMA", "action": { "id": "rode.wallpapers", "args": {} } }
                },
                "system": {
                    "config": { "modifiers": ["SUPER", "SHIFT"], "key": "C", "action": { "id": "rode.config", "args": {} } },
                    "lockscreen": { "modifiers": ["SUPER"], "key": "L", "action": { "id": "system.lock", "args": {} } },
                    "overview": { "modifiers": ["SUPER"], "key": "TAB", "action": { "id": "rode.overview", "args": {} } },
                    "powermenu": { "modifiers": ["SUPER"], "key": "ESCAPE", "action": { "id": "rode.powermenu", "args": {} } },
                    "tools": { "modifiers": ["SUPER"], "key": "S", "action": { "id": "rode.tools", "args": {} } },
                    "screenshot": { "modifiers": ["SUPER", "SHIFT"], "key": "S", "action": { "id": "rode.screenshot", "args": {} } },
                    "screenrecord": { "modifiers": ["SUPER", "SHIFT"], "key": "R", "action": { "id": "rode.screenrecord", "args": {} } },
                    "lens": { "modifiers": ["SUPER", "SHIFT"], "key": "A", "action": { "id": "rode.lens", "args": {} } },
                    "reload": { "modifiers": ["SUPER", "ALT"], "key": "B", "action": { "id": "rode.reload", "args": {} } },
                    "quit": { "modifiers": ["SUPER", "CTRL", "ALT"], "key": "B", "action": { "id": "rode.quit", "args": {} } }
                }
            }

            function getRodeDefault(section, key) {
                if (defaultRodeBinds[section] && defaultRodeBinds[section][key]) {
                    const bind = defaultRodeBinds[section][key];
                    return {
            ...
            property list<var> custom: [
                ...
                },
                {
                    ...
                    "actions": [
                        {
                            "dispatcher": "exec",
                            "argument": "rodctl monitor set-dpms 0 0",
                            "flags": "l",
                            "layouts": []
                        }
                    ],
                ...
                },
                {
                    ...
                    "actions": [
                        {
                            "dispatcher": "exec",
                            "argument": "rodctl monitor set-dpms 0 1",
                            "flags": "l",
                            "layouts": []
                        }
                    ],
