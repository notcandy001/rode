...
When changes are made to axctl (in `~/.local/src/axctl/`), manual build and install is required:

1. Build: `cd ~/.local/src/axctl && go build -o bin/axctl .`
...
## OVERVIEW
Rode is a highly customizable Wayland shell built with Quickshell. It provides a unified panel (bar, dock, notch), dashboard, lockscreen, desktop widgets, and notification system, driven by a reactive JSON configuration system. Multi-monitor support via `Variants` on `Quickshell.screens`.
...
| **Theme/Colors** | `modules/theme/Colors.qml` | Watches `~/.cache/rode/colors.json` reactively |
...
curl -L https://github.com/notcandy/rode/releases/latest/install.sh | sh
```
...
- `axctl` is a core part of this project. It abstracts compositor interactions. It is a core dependency and the source code is available at `~/.local/src/axctl/`.
- We register a changelog in a website. The local repo for this website is at `~/.local/src/rode-web/`. The changelog entries are stored in `content/rode/changelog/` as Zola markdown files. Write following the structure by referencing other entries, and add links to PRs and issues when relevant. Only write a changelog when the user asks for it.
