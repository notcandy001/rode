...
Item {
    ...
    readonly property bool isScreenFocused: RodctlService.focusedMonitor && RodctlService.focusedMonitor.name === screen.name

    // Monitor reference and refrence to toplevels on monitor
    readonly property var compositorMonitor: RodctlService.monitorFor(screen)
    readonly property var toplevels: (!compositorMonitor || !compositorMonitor.activeWorkspace || !RodctlService.clients.values) ? [] : RodctlService.clients.values.filter(c => c.workspace.id === compositorMonitor.activeWorkspace.id)
