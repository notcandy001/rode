...
Item {
    ...
    // Timer to reset override position after a delay (waiting for RodctlService update)
    Timer {
    ...
    MouseArea {
        ...
        onEntered: {
            ...
            if (root.windowData) {
                // Get current active workspace from RodctlService
                let currentWorkspace = RodctlService.focusedMonitor?.activeWorkspace?.id;
                ...
                if (currentWorkspace && windowWorkspace && currentWorkspace === windowWorkspace) {
                    RodctlService.dispatch(`focuswindow address:${windowData.address}`);
                }
            }
        }
            ...
            if (mouse.button === Qt.LeftButton) {
                ...
                if (targetWorkspace !== -1 && targetWorkspace !== windowData?.workspace.id) {
                    ...
                    if (windowData?.floating && (root.x !== root.initX || root.y !== root.initY)) {
                        ...
                        RodctlService.dispatch(`movetoworkspacesilent ${targetWorkspace}, address:${windowData?.address}`);
                        RodctlService.dispatch(`movewindowpixel exact ${percentageX}% ${percentageY}%, address:${windowData?.address}`);
