...
Singleton {
    ...
    // Config path for rodctl daemon
    property string configPath: (Quickshell.env("XDG_DATA_HOME") || (Quickshell.env("HOME") + "/.local/share")) + "/rode/rodctl.toml"
    ...
    function dispatch(command) {
        ...
        let finalCommand = ["rodctl"].concat(cmdArgs.filter(x => x !== "" && x !== undefined));
    ...
    property Process ensureConfigDir: Process {
        command: ["mkdir", "-p", (Quickshell.env("XDG_DATA_HOME") || (Quickshell.env("HOME") + "/.local/share")) + "/rode"]
        running: true
    }

    property Process rodctlProcess: Process {
        command: ["rodctl", "-c", root.configPath, "daemon"]
        running: true
        stdout: SplitParser {
            onRead: (data) => {
        ...
        onExited: (code) => {
            console.warn("rodctl daemon exited with code:", code)
        }
    }
    ...
    Timer {
        ...
        interval: 500
        running: true
        onTriggered: rodctlSubscribe.running = true
    }
    ...
    property Process rodctlSubscribe: Process {
        command: ["rodctl", "subscribe"]
        running: false
        stdout: SplitParser {
            onRead: (data) => {
                if (!data) return;
                try {
                    ...
                    console.error("RodctlService subscribe JSON parse error:", e);
                }
            }
        }
        onExited: (code) => {
            console.warn("rodctl subscribe exited:", code);
    ...
    Component.onDestruction: {
        ...
        rodctlProcess.running = false
        rodctlSubscribe.running = false
    }
}
