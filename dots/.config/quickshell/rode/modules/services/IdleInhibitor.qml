...
Item {
    ...
    function _createInhibitor() {
        var en = enabled ? 1 : 0;
        var cmd = "rodctl system idle-inhibitor-create " + en;
    ...
    function _destroyInhibitor() {
        if (_inhibitorId > 0) {
            var cmd = "rodctl system idle-inhibitor-destroy " + _inhibitorId;
    ...
    function _toggleInhibitor(enable) {
        if (_inhibitorId > 0) {
            var cmd = "rodctl system idle-inhibitor-set " + _inhibitorId + " " + (enable ? 1 : 0);
