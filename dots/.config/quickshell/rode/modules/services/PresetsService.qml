...
Singleton {
    ...
    readonly property string configDir: (Quickshell.env("XDG_CONFIG_HOME") || (Quickshell.env("HOME") + "/.config")) + "/rode"
    ...
    function savePreset(presetName: string, configFiles: var) {
        ...
        for (const configFile of configFiles) {
            ...
            // The source is configDir (~/.config/Rode), NOT configDir/config
            // But wait, the configDir property is defined as ~/.config/Rode below?
            // Let's check the property definition.
            // property string configDir: ... + "/Rode"
            // But Config.qml says configDir is ... + "/Rode/config"
            // We need to match Config.qml's path.
            
            // In Config.qml: property string configDir: ... + "/Rode/config"
            // Here: readonly property string configDir: ... + "/Rode"
            // This is a mismatch!
            
            // We should use the same path as Config.qml for reading/writing config files.
            // Let's assume the files are in .../Rode/config based on Config.qml and ls output.
