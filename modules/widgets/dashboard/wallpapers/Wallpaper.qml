...
PanelWindow {
    ...
    WlrLayershell.namespace: "rode:wallpaper"
    ...
    property string mpvShaderDir: Quickshell.env("HOME") + "/.cache/rode/mpv_shaders_" + (currentScreenName ? currentScreenName : "ALL")
    ...
    property string colorPresetsDir: Quickshell.env("HOME") + "/.config/rode/colors"
    ...
    function applyColorPreset() {
        ...
        var dest = Quickshell.env("HOME") + "/.cache/rode/colors.json";
    ...
    function getThumbnailPath(filePath) {
        ...
        var thumbnailPath = Quickshell.env("HOME") + "/.cache/rode" + "/thumbnails/" + relativeDir + "/" + thumbnailName;
    ...
    function getLockscreenFramePath(filePath) {
        ...
        if (fileType === 'video' || fileType === 'gif') {
            ...
            var cachePath = Quickshell.env("HOME") + "/.cache/rode" + "/lockscreen/" + fileName + ".jpg";
            return cachePath;
        }
    ...
    function generateLockscreenFrame(filePath) {
        ...
        var dataPath = Quickshell.env("HOME") + "/.cache/rode";
    ...
    // property string mpvSocket: "/tmp/rode_mpv_socket"
    property string mpvSocket: "/tmp/rode_mpv_socket_" + (currentScreenName ? currentScreenName : "ALL")
    ...
    Process {
        id: mpvSyncProcess
        running: false
        command: ["bash", "-c", "for sock in /tmp/rode_mpv_socket_*; do echo '{ \"command\": [\"set_property\", \"time-pos\", 0] }' | socat - \"$sock\" 2>/dev/null; done"]
        onExited: code => {
    ...
    FileView {
        ...
        path: Quickshell.env("HOME") + "/.cache/rode/wallpapers.json"
        watchChanges: true
    ...
    Process {
        ...
        command: ["test", "-f", Quickshell.env("HOME") + "/.cache/rode/wallpapers.json"]
    ...
    Process {
        ...
        command: ["python3", decodeURIComponent(Qt.resolvedUrl("../../../../scripts/thumbgen.py").toString().replace("file://", "")), Quickshell.env("HOME") + "/.cache/rode" + "/wallpapers.json", Quickshell.env("HOME") + "/.cache/rode", fallbackDir]
