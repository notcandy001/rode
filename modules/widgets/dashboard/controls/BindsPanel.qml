...
Item {
    ...
    property string currentCategory: "rode"
    ...
    function unbindKeybind(bind) {
        ...
        if (bind.keys && bind.keys.length > 0) {
            for (let k = 0; k < bind.keys.length; k++) {
                ...
                const command = `rodctl config unbind-key ${mods},${key}`;
    ...
    property bool isEditingRode: false
    ...
    function openEditDialog(bind, index, isRode) {
        root.editingIndex = index;
        root.editingBind = bind;
        root.isEditingRode = isRode;

        // Initialize edit form state
        if (isRode) {
            // Rode binds still use old format (single key)
    ...
    function saveEdit() {
        if (root.isEditingRode) {
            // Save rode bind (still uses old format internally)
            const path = root.editingBind.path.split(".");
            // path = ["rode", "section"?, "bindName"]
            
            const adapter = Config.keybindsLoader.adapter;
            if (adapter && adapter.rode) {
                let bindObj = null;
                if (path.length === 2) {
                    // Top level: rode.bindName
                    bindObj = adapter.rode[path[1]];
                } else if (path.length === 3) {
                    // Nested: rode.system.bindName
                    bindObj = adapter.rode[path[1]][path[2]];
                }
    ...
    // Get rode binds as a flat list
    function getRodeBinds() {
        const adapter = Config.keybindsLoader.adapter;
        if (!adapter || !adapter.rode)
            return [];

        const binds = [];
        const rode = adapter.rode;

        // Core Rode binds (Launcher, Dashboard, etc.)
            ...
            for (const key of systemKeys) {
                if (rode.system[key]) {
                    binds.push({
                        category: "System",
                        name: key.charAt(0).toUpperCase() + key.slice(1),
                        path: "rode.system." + key,
                        bind: rode.system[key]
                    });
                }
            }
        }
    ...
    Flickable {
        ...
        ColumnLayout {
            ...
            Repeater {
                id: rodeRepeater
                model: root.currentCategory === "rode" ? root.getRodeBinds() : []
            ...
            Text {
                ...
                Layout.topMargin: 20
                visible: (root.currentCategory === "rode" && rodeRepeater.count === 0) || (root.currentCategory === "custom" && customRepeater.count === 0)
                text: root.currentCategory === "rode" ? "No Rode binds configured" : "No custom binds configured"
                font.family: Config.theme.font
                font.pixelSize: Styling.fontSize(0)
            ...
            ColumnLayout {
                ...
                Item {
                    ...
                    RowLayout {
                        ...
                        StyledRect {
                            id: deleteButton
                            visible: !root.isEditingRode && !root.isCreatingNew
                        ...
                        // Reset button (only for Rode binds)
                        StyledRect {
                            id: resetButton
                            visible: root.isEditingRode
                            ...
                            MouseArea {
                                ...
                                cursorShape: Qt.PointingHandCursor
                                onClicked: {
                                    if (root.isEditingRode && root.editingBind) {
                                        const path = root.editingBind.path.split(".");
                                        // path = ["rode", "dashboard"|"system", "bindName"]
                                        const section = path[1];
                                        const bindName = path[2];
                                        
                                        // Use the new helper in Config.qml to get the default values
                                        const defaultBind = Config.keybindsLoader.adapter.getRodeDefault(section, bindName);
                ...
                Item {
                    ...
                    ColumnLayout {
                        ...
                        // Bind name/info (for rode binds only)
                        Text {
                            visible: root.isEditingRode && root.editingBind !== null
                        ...
                        ColumnLayout {
                            ...
                            RowLayout {
                                ...
                                StyledRect {
                                    id: removeKeyBtn
                                    visible: root.editKeys.length > 1 && !root.isEditingRode
                        ...
                        // =====================
                        // ACTIONS SECTION (custom binds & flags for rode)
                        // =====================
                        ColumnLayout {
                            Layout.fillWidth: true
                            spacing: 8
                            // visible: !root.isEditingRode - Removed to allow editing flags for Rode binds
                            ...
                            RowLayout {
                                ...
                                // Page indicator
                                Text {
                                    visible: root.editActions.length > 1 && !root.isEditingRode
                            ...
                            // =====================
                            // LAYOUT SELECTOR (for RodctlService)
                            // =====================
                            Text {
                                text: "Layouts (RodctlService)"
                                font.family: Config.theme.font
                                font.pixelSize: Styling.fontSize(-1)
                                font.weight: Font.Medium
                                color: Colors.overSurfaceVariant
