...
PanelWindow {
    ...
    WlrLayershell.namespace: "rode:overview"
                ...
                SearchInput {
                    ...
                    onTabPressed: {
                        if (searchInput.text.length === 0) {
                            const current = RodctlService.focusedWorkspace?.id || 1;
                            const next = current + 1;
                            if (next > Config.workspaces.shown) {
                                RodctlService.dispatch("workspace 1");
                            } else {
                    ...
                    onShiftTabPressed: {
                        if (searchInput.text.length === 0) {
                            ...
                            if (prev < 1) {
                                RodctlService.dispatch("workspace " + Config.workspaces.shown);
                            } else {
                                RodctlService.dispatch("workspace r-1");
                            }
                        } else if (overviewLoader.item) {
