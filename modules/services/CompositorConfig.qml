...
QtObject {
    ...
    property Process readAnimationsProcess: Process {
        command: ["rodctl", "config", "get-animations"]
        stdout: StdioCollector {
            onStreamFinished: {
                try {
                    const parsed = JSON.parse(text);
                    if (Array.isArray(parsed) && parsed.length > 0) {
                        // rodctl config get-animations returns [animations, beziers]
    ...
    function formatColorForCompositor(color) {
        // RodctlService expects colors in format: rgb(rrggbb) or rgba(rrggbbaa)
    ...
    function applyCompositorConfigInternal() {
        ...
        if (!GlobalStates.compositorLayoutReady) {
            console.log("CompositorConfig: Esperando que se detecte el layout de RodctlService...");
            return;
        }
