...
/**
 * CompositorTomlWriter - Generates TOML configuration for rodctl
 * Writes to ~/.local/share/rode/rodctl.toml
 */
Singleton {
    id: root

    property string outputPath: (Quickshell.env("XDG_DATA_HOME") || (Quickshell.env("HOME") + "/.local/share")) + "/rode/rodctl.toml"
    ...
    function generateToml() {
        ...
        if (Config.keybindsLoader.loaded && Config.keybindsLoader.adapter) {
            ...
            if (rode) {
                pushCoreBind(rode.launcher);
                pushCoreBind(rode.dashboard);
                pushCoreBind(rode.assistant);
                pushCoreBind(rode.clipboard);
                pushCoreBind(rode.emoji);
                pushCoreBind(rode.notes);
                pushCoreBind(rode.tmux);
                pushCoreBind(rode.wallpapers);

                if (rode.system) {
                    pushCoreBind(rode.system.overview);
                    pushCoreBind(rode.system.powermenu);
                    pushCoreBind(rode.system.config);
                    pushCoreBind(rode.system.lockscreen);
                    pushCoreBind(rode.system.tools);
                    pushCoreBind(rode.system.screenshot);
                    pushCoreBind(rode.system.screenrecord);
                    pushCoreBind(rode.system.lens);
                    if (rode.system.reload) pushCoreBind(rode.system.reload);
                    if (rode.system.quit) pushCoreBind(rode.system.quit);
                }
            }
