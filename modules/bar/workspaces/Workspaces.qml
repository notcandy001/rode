...
Item {
    ...
    readonly property var monitor: RodctlService.monitorFor(bar.screen)
    ...
    function updateWorkspaceOccupied() {
        if (Config.workspaces.dynamic) {
            ...
            const occupiedIds = RodctlService.workspaces.values.filter(ws => CompositorData.workspaceOccupationMap[ws.id]).map(ws => ws.id).sort((a, b) => a - b).slice(0, Config.workspaces.shown);
    ...
    Connections {
        target: RodctlService.workspaces
    ...
    WheelHandler {
        onWheel: event => {
            if (event.angleDelta.y < 0)
                RodctlService.dispatch(`workspace r+1`);
    ...
    MouseArea {
        ...
        onPressed: event => {
            if (event.button === Qt.BackButton) {
                RodctlService.dispatch(`togglespecialworkspace`);
            }
        }
    }
    ...
    RowLayout {
        ...
        Repeater {
            ...
            Button {
                ...
                onPressed: RodctlService.dispatch(`workspace ${workspaceValue}`)
                width: workspaceButtonWidth
