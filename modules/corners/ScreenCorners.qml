...
PanelWindow {
    ...
    function updateFullscreen() {
        const mon = RodctlService.monitorFor(screen);
        if (mon) {
            monitor = mon;
        }
        ...
        if (toplevel && toplevel.fullscreen && RodctlService.focusedMonitor && RodctlService.focusedMonitor.id === monId) {
    ...
    WlrLayershell.namespace: "rode:screenCorners"
